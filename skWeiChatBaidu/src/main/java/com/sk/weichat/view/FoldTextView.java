    ...
    private void translateText(Layout layout, BufferType type) {
        ...
        if (layout.getLineCount() > mShowMaxLine) {
            ...
            if (mTipGravity == END) {
                ...
                // 文本带有换行\n可能导致换行判断异常，所以单独拿出来判断处理，
                while (mOriginalText.charAt(end) != '\n' && end - 1 < mOriginalText.length() && layout.getPrimaryHorizontal(end - 1) + getTextWidth(mOriginalText.subSequence(end - 1, end).toString()) < x) {
                    end++;
                }
                if (mOriginalText.charAt(end) != '\n') {
                    end--;
                }
            } else {
                end--;
