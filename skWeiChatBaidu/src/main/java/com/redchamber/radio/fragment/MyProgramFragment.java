...
import com.redchamber.radio.adapter.MyProgramAdapter;
...
public class MyProgramFragment extends BaseFragment {
    ...
    protected int setLayout() {
        return R.layout.red_fragment_my_program;
    }

    @Override
    protected void initView() {
        mAdapter = new MyProgramAdapter(null);
        mRvProgram.setLayoutManager(new LinearLayoutManager(getContext()));
        mRvProgram.setAdapter(mAdapter);
        initRefreshLayout();
        initOnClick();
        getMessageList();
    }
    ...
    private void initOnClick() {
        mAdapter.setBtnOnClice(new MyProgramAdapter.BtnOnCommClick() {
            @Override
            public void btnOnCommClick(PageDataBean pageDataBean, int fuctionType, int Position, View view) {
                 if(fuctionType==3){
                    stopYueba(pageDataBean,Position);
                    return;
                }

                ReportPopupWindow reportPopupWindow=new ReportPopupWindow(getActivity(),pageDataBean.getDiscussFlag()==0?"开启评论":"禁止评论","删除节目",view);
                ...
                .execute(new BaseCallback<String>(String.class) {
                    ...
                    public void onResponse(ObjectResult<String> result) {
                        if (result.getResultCode() == 1) {
                            ...
                            mAdapter.notifyDataSetChanged();
                            ToastUtil.showLongToast(getActivity(),"删除节目成功");
                        }
                    }
                    @Override
                    public void onError(Call call, Exception e) {
     ...
     * @param position
     */
    public void stopYueba(PageDataBean pageDataBean, int position) {
        HashMap<String, String> params = new HashMap<String, String>();
        params.put("access_token", coreManager.getSelfStatus().accessToken);
        params.put("programId", pageDataBean.getProgramId());
        HttpUtils.post().url(coreManager.getConfig().RED_MY_YUEBA_STOP)
                .params(params)
                .build()
                .execute(new BaseCallback<String>(String.class) {
                    ...
                    public void onResponse(ObjectResult<String> result) {
                        if (result.getResultCode() == 1) {
                         /*   mAdapter.remove(position);
                            mAdapter.notifyDataSetChanged();*/
                            ToastUtil.showLongToast(getActivity(),"停止报名成功");
                        }else {
                            ToastUtil.showLongToast(getActivity(),result.getResultMsg());
                        }
                    }
                    @Override
                    public void onError(Call call, Exception e) {
