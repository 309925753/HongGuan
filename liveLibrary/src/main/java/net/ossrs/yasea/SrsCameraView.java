...
public class SrsCameraView extends GLSurfaceView implements GLSurfaceView.Renderer {
    ...
    public SrsCameraView(Context context, AttributeSet attrs) {
        ...
        setEGLContextClientVersion(2);//系统api设置opengl版本
        setRenderer(this);//设置渲染器
        setRenderMode(GLSurfaceView.RENDERMODE_WHEN_DIRTY);//设置渲染模式 脏模式
    }
    ...
    public boolean startCamera() {
        ...
        Camera.Parameters params = mCamera.getParameters();//配置camera参数
        params.setPictureSize(mPreviewWidth, mPreviewHeight);//设置图片大小
        params.setPreviewSize(mPreviewWidth, mPreviewHeight);//设置预览界面大小
        int[] range = adaptFpsRange(SrsEncoder.VFPS, params.getSupportedPreviewFpsRange());
        params.setPreviewFpsRange(range[0], range[1]);//设置fps
        params.setPreviewFormat(ImageFormat.NV21);//设置原始的NV21编码  通过onpreviewframe回调获取到每一帧的NV21  然后编码成YUv420 然后添加特效和滤镜通过opengl纹理渲染，然后编码成H264
        params.setFlashMode(Camera.Parameters.FLASH_MODE_OFF);//设置闪光灯
        params.setWhiteBalance(Camera.Parameters.WHITE_BALANCE_AUTO);//设置普光
        params.setSceneMode(Camera.Parameters.SCENE_MODE_AUTO);

        List<String> supportedFocusModes = params.getSupportedFocusModes();//获取到聚焦的支持
        ...
        List<String> supportedFlashModes = params.getSupportedFlashModes();//获取到闪光灯支持
        ...
        mCamera.setDisplayOrientation(mPreviewRotation);// 设置角度

        try {
            mCamera.setPreviewTexture(surfaceTexture);  // 设置预览界面
        } catch (IOException e) {
            e.printStackTrace();
        }
