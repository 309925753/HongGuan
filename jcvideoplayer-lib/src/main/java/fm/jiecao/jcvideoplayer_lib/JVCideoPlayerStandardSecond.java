...
public class JVCideoPlayerStandardSecond extends JCVideoPlayer {
    ...
    private void downloadUrl(String mVideoPath) {
        ...
        if (mVideoPath.startsWith("http")) {
            File innerFile = new File(mContext.getFilesDir(), "external");
            File videoFolder;
            videoFolder = mContext.getExternalFilesDir(Environment.DIRECTORY_MOVIES);
            if (videoFolder != null && !videoFolder.exists()) {
                videoFolder.mkdirs();
            }
            if (videoFolder == null) {
                // 不能为空，
                videoFolder = new File(innerFile, Environment.DIRECTORY_MOVIES);
            }
            String path = videoFolder.getAbsolutePath() + "/" +
